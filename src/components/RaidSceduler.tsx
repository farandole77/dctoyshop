...
export default function RaidScheduler() {
  ...
  const handleWritePost = async () => {
    ...
    try {
      ...
      if (selectedImage) {
        ...
        const imgName = `img_${Date.now()}.${fileExt}`;
      ...
      if (selectedFile) {
        ...
        const fName = `file_${Date.now()}.${fileExt}`;
        const { error: uploadError } = await supabase.storage.from('files').upload(fName, selectedFile);
        if (uploadError) throw uploadError;
        const { data } = supabase.storage.from('files').getPublicUrl(fName);
      ...
      await supabase.from('posts').insert([{ 
        ...
        user_id: user.id, 
        image_url: imageUrl,
        file_url: fileUrl, // 추가됨
        file_name: fileNameStr // 추가됨
      }]);

      setPostTitle(''); setPostContent(''); 
      setSelectedImage(null); setPreviewUrl(''); setSelectedFile(null);
      setIsWriteModalOpen(false); 
      fetchPosts();
    } catch (error) { console.error(error); alert("업로드 오류 발생"); } finally { setUploading(false); }
  };
  ...
  const handleCreate = async () => { if (!selectedDungeon) return alert('던전 선택!'); const typeTag = raidType === 'abyss' ? '[어비스]' : '[레이드]'; const finalTitle = `${typeTag} ${selectedDungeon}`; await supabase.from('raids').insert([{ title: finalTitle, start_time: selectedDate, created_by_email: user.email, max_members: maxMembers, host_name: myProfile?.nickname || '알수없음', host_avatar: myProfile?.game_class || '모험가' }]); setRaidTitle(''); setMaxMembers(4); setIsCreateModalOpen(false); fetchRaids(); };
  ...
  return (
    <div className="flex flex-col h-screen bg-[#f3f4f6] overflow-hidden">
      <header className="h-16 md:h-20 bg-white border-b border-gray-200 flex items-center justify-between px-4 md:px-10 shrink-0 z-30 shadow-sm relative">
        ...
        <nav className="hidden md:flex items-center gap-2 bg-gray-50 p-1.5 rounded-full border border-gray-100"><TabButton tabName="calendar" label="일정" icon={<Icons.Calendar />} /><TabButton tabName="stats" label="통계" icon={<Icons.Chart />} /><TabButton tabName="board" label="팁" icon={<Icons.Board />} />{/* <TabButton tabName="voice" label="보이스" icon={<Icons.Mic />} /> */}{isAdmin && <TabButton tabName="admin" label="관리" icon={<Icons.Admin />} />}</nav>
      ...
      <main className="flex-1 overflow-y-auto p-4 md:p-8 max-w-7xl mx-auto w-full pb-24 md:pb-8 relative">
        {activeTab === 'calendar' ? (
          ...
          <div className="relative min-h-full pb-20">
            ...
            {post.image_url && (<div className="mb-4 rounded-xl overflow-hidden shadow-sm border border-gray-100"><img src={post.image_url} alt="첨부 이미지" className="w-full object-cover max-h-80" /></div>)}
            ...
            {post.file_url && (
              <a href={post.file_url} target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 p-3 mb-4 bg-gray-50 rounded-xl border border-gray-200 hover:bg-indigo-50 hover:border-indigo-200 transition-colors group/file">
                ...
                <div className="flex-1 overflow-hidden">
                  <div className="text-sm font-bold text-gray-700 truncate group-hover/file:text-indigo-700">{post.file_name || '첨부파일'}</div>
                  <div className="text-xs text-gray-400">클릭하여 다운로드</div>
                </div>
                <div className="text-gray-400 group-hover/file:text-indigo-500"><Icons.Download /></div>
              </a>
            )}
          ...
          <div className="bg-white p-8 rounded-[2rem] shadow-2xl w-full max-w-md relative">
            ...
            <div className="mb-6 flex gap-2">
              ...
              <div className="flex-1">
                <input type="file" accept="image/*" id="img-upload" className="hidden" onChange={handleImageSelect} />
                <label htmlFor="img-upload" className="flex items-center justify-center gap-2 w-full py-3 bg-gray-100 rounded-xl cursor-pointer hover:bg-gray-200 transition-all text-xs font-bold border border-dashed border-gray-300">
                  <Icons.Camera /> {selectedImage ? '사진 변경' : '사진 첨부'}
                </label>
              </div>
              {/* 파일 첨부 */}
              <div className="flex-1">
                <input type="file" id="file-upload" className="hidden" onChange={handleFileSelect} />
                <label htmlFor="file-upload" className="flex items-center justify-center gap-2 w-full py-3 bg-gray-100 rounded-xl cursor-pointer hover:bg-gray-200 transition-all text-xs font-bold border border-dashed border-gray-300">
                  <Icons.Clip /> {selectedFile ? '파일 변경' : '파일 첨부'}
                </label>
              </div>
            </div>
              ...
              <div className="mb-6 space-y-2">
                {previewUrl && (
                  <div className="relative w-full h-32 rounded-xl overflow-hidden border border-gray-200">
                    ...
                    <button onClick={() => { setSelectedImage(null); setPreviewUrl(''); }} className="absolute top-1 right-1 bg-black/50 text-white rounded-full p-1"><Icons.Close /></button>
                  </div>
                )}
                {selectedFile && (
                  <div className="flex items-center justify-between bg-indigo-50 p-3 rounded-xl border border-indigo-100">
                    ...
                    <button onClick={() => setSelectedFile(null)} className="text-gray-400 hover:text-red-500"><Icons.Close /></button>
                  </div>
                )}
              </div>
            )}
