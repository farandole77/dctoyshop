...
export default function RaidScheduler() {
  ...
  // ★ 탭에 'admin' 추가
  const [activeTab, setActiveTab] = useState<'calendar' | 'stats' | 'board' | 'admin'>('calendar');
  ...
  useEffect(() => {
    ...
    if (activeTab === 'admin') fetchAllProfiles(); // 관리자 탭이면 회원목록 불러오기
  }, [activeTab, statsFilter]);

  useEffect(() => { if (isCreateModalOpen) setSelectedDungeon(DUNGEON_DATA[raidType][0]); }, [raidType, isCreateModalOpen]);
  ...
  useEffect(() => {
    ...
    return () => { clearTimeout(timer); window.removeEventListener('resize', changeView); };
  }, [activeTab]);
  ...
  const handleDeleteMember = async (memberId: string, memberName: string) => {
    if (!confirm(`정말 '${memberName}' 회원을 강퇴(삭제)하시겠습니까?\n모든 데이터가 사라집니다.`)) return;
    
    // 프로필 삭제 (CASCADE로 인해 관련 글, 레이드, 참가기록도 다 삭제됨)
    await supabase.from('profiles').delete().eq('id', memberId);
    alert("삭제되었습니다.");
    fetchAllProfiles(); // 목록 갱신
  };
  ...
  const handleWritePost = async () => {
    if (!postTitle || !postContent) return alert("입력해주세요.");
    setUploading(true); let imageUrl = null;
    try {
      if (selectedImage) {
        const fileExt = selectedImage.name.split('.').pop(); const fileName = `${Date.now()}.${fileExt}`;
        const { error: uploadError } = await supabase.storage.from('images').upload(fileName, selectedImage);
        if (uploadError) throw uploadError; const { data } = supabase.storage.from('images').getPublicUrl(fileName); imageUrl = data.publicUrl;
      }
  ...
  const handleAddToCalendar = () => {
    ...
    const details = encodeURIComponent("던컨의 장난감가게 일정");
  ...
  // ★ 관리자(isAdmin)이거나 작성자면 true (삭제 권한)
              ...
              {allProfiles.map(member => (
                <div key={member.id} className="flex items-center justify-between bg-gray-50 p-4 rounded-2xl border border-gray-100">
                  <div className="flex items-center gap-4">
                    {renderAvatar(member.game_class, "w-10 h-10")}
                    <div>
                      <div className="font-bold text-gray-900 flex items-center gap-2">
                        {member.nickname} 
                        {member.role === 'admin' && <span className="bg-black text-white text-[10px] px-2 py-0.5 rounded-full">ADMIN</span>}
                      </div>
                      <div className="text-xs text-gray-500">{member.game_class}</div>
                    </div>
                  </div>
                  {/* 본인이 아니고 관리자가 아니면 강퇴 버튼 표시 */}
